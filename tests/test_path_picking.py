"""Tests for SimStrategyAgent map fork routing — probe-based path selection."""
...
class TestMapForkRouting:
    """Test pick_branch routing logic via walk_map."""

    def test_walk_returns_valid_path(self):
        """Walking the map returns a non-empty path."""
        sts_map = _make_simple_map()
        char = Character(player_hp=80, player_max_hp=80)
        agent = SimStrategyAgent(sim_nodes=100, sim_sims=100)
        path = walk_map(agent, sts_map, char, seed=42)
        assert len(path) >= 2
        assert path[0][0] == 0
    ...
    def test_on_full_act1_map(self):
        """Routing works on a full 15-floor map plus the boss at floor 15."""
        sts_map = generate_act1_map(seed=123)
        char = Character(player_hp=80, player_max_hp=80)
        agent = SimStrategyAgent(sim_nodes=100, sim_sims=100)
        path = walk_map(agent, sts_map, char, seed=123)
        assert path[-1][0] == 15
        for i in range(1, len(path)):
            assert path[i][0] == path[i - 1][0] + 1
        for f, x in path:
    ...
    def test_produces_valid_path_to_boss(self):
        """Walking produces a valid, contiguous path reaching the boss."""
        sts_map = generate_act1_map(seed=99)
        char = Character(player_hp=80, player_max_hp=80)
        agent = SimStrategyAgent(sim_nodes=100, sim_sims=100)
        path = walk_map(agent, sts_map, char, seed=99)
        assert path[-1][0] == 15
        for i in range(1, len(path)):
            assert path[i][0] == path[i - 1][0] + 1
        for f, x in path:
    ...
    def test_run_act1_uses_strategy_agent_routing(self):
